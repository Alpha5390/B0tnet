...
# Токены и настройки
TOKEN = '7880059689:AAEghaJ0_JoekZRz8SFLwBdtTiTZ5X46h5Y'
CRYPTO_TOKEN = '353631:AA7L7DhUoI6fLo5TrP0hc3zY8yJGEq1eaOf'
ADMIN_ID = 7601411293
API_ID = 29931460
API_HASH = "6c1101e57d041abd1ca032f7079ede37"
...
def get_user_data(user_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT balance, subscription, sub_end_date, next_snuser_usage FROM users WHERE user_id = ?', (user_id,))
    result = c.fetchone()
    if result is None:
...
def set_subscription(user_id, days):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT subscription, sub_end_date FROM users WHERE user_id = ?', (user_id,))
    current = c.fetchone()
    ...
    c.execute('UPDATE users SET subscription = 1, sub_end_date = ? WHERE user_id = ?', 
...
def get_active_subscriptions():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM users WHERE subscription = 1 AND sub_end_date > ?", 
...
def get_active_paid_payments(limit=5):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT user_id, amount, days, status, created_at FROM payments WHERE status IN ('pending', 'paid') ORDER BY created_at DESC LIMIT ?", (limit,))
    payments = c.fetchall()
    conn.close()
    return payments
...
def save_recovery_request(user_id, phone_number, status):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('INSERT INTO recovery_requests (user_id, phone_number, status, created_at) VALUES (?, ?, ?, ?)',
...
def check_recovery_cooldown(user_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT created_at, status FROM recovery_requests WHERE user_id = ? ORDER BY created_at DESC LIMIT 1", (user_id,))
...
def update_snosing_cooldown(user_id):
    ...
    c.execute('UPDATE users SET next_snuser_usage = ? WHERE user_id = ?', (next_usage, user_id))
    conn.commit()
    conn.close()
    return next_usage
...
def get_bot_status():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT setting_value FROM bot_settings WHERE setting_key = "bot_enabled"')
    result = c.fetchone()
    conn.close()
...
async def send_recovery_request(phone_number, recovery_data, index):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=data, headers=headers, timeout=aiohttp.ClientTimeout(total=20), ssl=False) as response:
                if response.status in [200, 201, 202, 204]:
                    print(f"{Fore.GREEN}[#{index+1}] Успешно отправлено!{Style.RESET_ALL}")
                    return "sent"
                else:
                    print(f"{Fore.RED}[-] Ошибка: код {response.status}{Style.RESET_ALL}")
                    return "failed"
...
async def send_report_message(link: str, api_id: int, api_hash: str) -> dict:
    ...
    try:
        ...
        print(f"{Fore.RED}Не удалось открыть папку с сессиями {SESSIONS_PATH}: {e}{Style.RESET_ALL}")
        return {"success": 0, "failed": 1}
    ...
    for session in sessions:
        ...
        try:
            ...
            if not await client.is_user_authorized():
                ...
                print(f"{Fore.RED}Сессия {session} не авторизована, переходим к следующей{Style.RESET_ALL}")
                failed_count += 1
            ...
            try:
                ...
                print(f"{Fore.RED}Не удалось найти чат {chat} для {session}: {e}, переходим к следующей сессии{Style.RESET_ALL}")
                failed_count += 1
                await client.disconnect()
                continue
            ...
            print(f"{Fore.YELLOW}Отправка жалобы от {session} на сообщение {message_id}{Style.RESET_ALL}")
            ...
            print(f"{Fore.GREEN}Жалоба от {session} на {link} успешно отправлена{Style.RESET_ALL}")
            ...
            print(f"{Fore.RED}Тайм-аут при работе с {session}, переходим к следующей сессии{Style.RESET_ALL}")
        
        except Exception as e:
            failed_count += 1
            logging.error(f"Ошибка при работе с {session} на {link}: {e}")
            print(f"{Fore.RED}Ошибка при работе с {session} на {link}: {e}, переходим к следующей сессии{Style.RESET_ALL}")
    ...
    logging.info(f"Отправка жалоб для {link} завершена: Успешно={success_count}, Неудачно={failed_count}")
    print(f"{Fore.CYAN}Отправка жалоб для {link} завершена: Успешно={success_count}, Неудачно={failed_count}{Style.RESET_ALL}")
    return {"success": success_count, "failed": failed_count}

@dp.message(Command("start"))
async def send_welcome(message: types.Message):
    if not BOT_ENABLED:
        await message.answer("<b>🤖 Бот временно отключен администратором!</b>", parse_mode=ParseMode.HTML)
        return
...
async def admin_panel(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        await message.answer("🔒 <b>У вас нет доступа к админ-панели!</b>", parse_mode=ParseMode.HTML)
        return
    ...
    admin_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎁 Выдать подписку", callback_data="admin_give_sub"),
         InlineKeyboardButton(text="🗑 Удалить счета", callback_data="admin_delete_invoices")],
    ...
    await message.answer(admin_text, parse_mode=ParseMode.HTML, reply_markup=admin_keyboard)
...
async def process_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    is_admin_action = callback.data in ["admin_give_sub", "admin_delete_invoices", "admin_payments", "admin_users", "toggle_bot", "admin_panel"] or \
                     callback.data.startswith("view_user_") or callback.data.startswith("give_sub_") or \
                     callback.data.startswith("set_sub_") or callback.data.startswith("user_payments_")
    
    if not BOT_ENABLED and not is_admin_action and callback.data != "back":
        await callback.answer("🤖 Бот временно отключен администратором!", show_alert=True)
        return
    ...
    if callback.data == "profile":
        ...
        profile_text, profile_keyboard = get_profile_menu(username, user_id, balance, subscription, sub_end_date)
        ...
        tasks_keyboard = InlineKeyboardMarkup(inline_keyboard=[ 
            [InlineKeyboardButton(text="🛠 Сносинг", callback_data="snosing"),
             InlineKeyboardButton(text="🔄 Восстановление", callback_data="recovery")],
            [InlineKeyboardButton(text="🔙 Назад", callback_data="back")]
        ])
        await bot.send_message(callback.message.chat.id, tasks_text, 
                            parse_mode=ParseMode.HTML, reply_markup=tasks_keyboard)
        ...
        subscribe_keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="5 дней (5$) 📆", callback_data="sub_5"),
             InlineKeyboardButton(text="10 дней (10$) 📆", callback_data="sub_10")],
            [InlineKeyboardButton(text="🔙 Назад", callback_data="back")]
        ])
        await bot.send_message(callback.message.chat.id, subscribe_text,
                            parse_mode=ParseMode.HTML, reply_markup=subscribe_keyboard)
        ...
        c.execute('INSERT OR REPLACE INTO payments (check_id, user_id, amount, days, status, created_at) VALUES (?, ?, ?, ?, ?, ?)',
        ...
        payment_keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🤍 Оплатить", url=pay_url),
             InlineKeyboardButton(text="🖤 Проверить", callback_data=f"check_sub_{invoice_id}")],
            [InlineKeyboardButton(text="❌ Отменить", callback_data="cancel_sub")]
        ])
        
        msg = await bot.send_message(callback.message.chat.id, payment_text,
                                  parse_mode=ParseMode.HTML, reply_markup=payment_keyboard)
        asyncio.create_task(check_payment_after_delay(callback, invoice_id, days, amount, msg.message_id))

    elif callback.data.startswith("check_sub_"):
        invoice_id = callback.data.split("_")[2]
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT user_id, days, amount, status FROM payments WHERE check_id = ?', (invoice_id,))
        payment = c.fetchone()
        conn.close()
        ...
        if payment and payment[3] == 'pending':
            ...
            if invoice_status.status == "paid":
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
                c.execute("UPDATE payments SET status = 'paid' WHERE check_id = ?", (invoice_id,))
                ...
                await callback.answer("💔 <b>Ошибка! Счет не оплачен!</b>", show_alert=True)
        ...
        payment_text = "💸 <b>Активные и оплаченные платежи</b>\n══════════════════════\n"
        if not payments:
        ...
        for user_id in users[:10]:
            keyboard.inline_keyboard.append([InlineKeyboardButton(text=f"ID: {user_id}", callback_data=f"view_user_{user_id}")])
        ...
        sub_status = "✅ Активна" if check_subscription(target_user_id) else "❌ Не активна"
        user_info = (
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🎁 Выдать подписку", callback_data=f"give_sub_{target_user_id}"),
             InlineKeyboardButton(text="💸 Платежи", callback_data=f"user_payments_{target_user_id}")],
            [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_users")]
        ])
        await bot.delete_message(callback.message.chat.id, callback.message.message_id)
        await bot.send_message(callback.message.chat.id, user_info,
            parse_mode=ParseMode.HTML,
            reply_markup=keyboard)
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="1 день", callback_data=f"set_sub_{target_user_id}_1"),
             InlineKeyboardButton(text="3 дня", callback_data=f"set_sub_{target_user_id}_3")],
            [InlineKeyboardButton(text="5 дней", callback_data=f"set_sub_{target_user_id}_5"),
             InlineKeyboardButton(text="10 дней", callback_data=f"set_sub_{target_user_id}_10")],
            [InlineKeyboardButton(text="🔙 Назад", callback_data=f"view_user_{target_user_id}")]
        ])
        await bot.delete_message(callback.message.chat.id, callback.message.message_id)
        await bot.send_message(callback.message.chat.id,
            sub_text,
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT user_id, amount, days, status, created_at FROM payments WHERE user_id = ? AND status IN ('pending', 'paid') ORDER BY created_at DESC LIMIT 5", (target_user_id,))
        payments = c.fetchall()
        conn.close()
...
async def check_payment_after_delay(callback: types.CallbackQuery, invoice_id: str, days: int, amount: float, message_id: int):
    ...
    if status and status[0] == 'pending':
        ...
        if invoice_status.status == "paid":
            ...
            c.execute("UPDATE payments SET status = 'expired' WHERE check_id = ?", (invoice_id,))
            conn.commit()
            try:
                await bot.delete_message(callback.message.chat.id, message_id)
            except:
...
async def give_subscription(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        await message.answer("🔒 <b>У вас нет прав на эту команду!</b>", parse_mode=ParseMode.HTML)
        return
    try:
        ...
        await message.answer(f"❌ <b>Ошибка:</b> {str(e)}. Использование: /give_sub <user_id> <days>", parse_mode=ParseMode.HTML)
    except Exception:
        pass
